...
| `firstmoves.js` | Where the three presses a stranger is offered actually sit, and the walk that says why they moved. `👋 Meet somebody` has carried a comment since v1.123 calling it *the one button a first-time visitor should press*, and at 390 × 844 it stood at **3,692 px of a 4,815 px document** — below every panel, because `@media (max-width: 960px)` stacks the aside second and every control added to that panel since inherited a position nobody chose. The defect was invisible to everything here: `node --test` cannot lay out a page, `targetsize.js` asks how big a control is and what is near it, `legibility.js` asks whether it can be read, and nine tests ask whether this one exists — **no instrument in this project has an axis for where in the queue a thing sits.** Holds `FIRST_MOVES` (each control with the visitor's *question* it answers, because the question decides the order and the function decided the old one), `WALK` (both viewports, before and after, including the 623 px this cost the desktop), `inMainColumn` (a string search over the shipped markup rather than a parsed copy of it, for `targetsize.js`'s reason), and `depthShare`. Pure observer: no DOM, no world, no RNG. | `test/firstmoves.test.js` |
...
| `chronicle.js` | Records notable events into a natural-history timeline (observation only). Since v1.136 a line can carry **which family** it is about (`sp`) as well as which animal (`who`), and only one of the two is outside the narration's hash — the difference is one `let`: a creature id comes from a counter at module scope, so two identical ponds in one process deal the same animals different numbers, while a species id comes from `Phylogeny.nextId`, a field born with the world. A line pointing at the wrong lineage is therefore a difference the channel is meant to catch. | — |
...
| `lifeline.js` | **The pond's whole run as one small line, beside the sentence that describes it.** v1.149's switch starts every visit on the quiet side, and counting what is on each side of it turned up the release's one unwritten consequence: **Simple keeps seven panels of prose and hides all five figures**, so the visitor least likely to read seven panels is the one handed nothing but reading. This is the smallest repair — a figure with no axis, no legend and no key, drawn in the empty half of the headline band, saying the one thing about a pond everybody already knows how to read. Whole run rather than a window, because a sweep found the line is a hill and not a ramp on **11 of 12 seeds by tick 7,200**, and because two of the twelve fall to six animals and come back — a comeback a recent window draws as a brand-new pond. The peak is read off the archive's min/max envelope rather than the sampled `pop`, which is the one thing exact under thinning; and the caption names only the start and the high-water mark, because it shares a bordered box with a sentence that **holds** — measured at a median of 180 ticks old, by which time the pond has moved a median of 7 animals — so a live count in the words would be two present-tense numbers disagreeing in one box. The ink says the moment; the words say the scale. Pure observer: no DOM, no world state, no random number, and no colour of its own. | `test/lifeline.test.js` |
...
| `eyeview.js` | **The pond as the animal has it, which is almost none of the pond.** The app had spent thirty releases keeping the front door's promise from the outside — what the crowd is doing, what has changed since tick one, who is worth watching — and the only surface that ever showed a *mind* was the inspector's weight strip, which is a bar chart of 243 numbers behind v1.149's switch. This is the other way to show one: not what the brain is made of, but what it is looking at. `Creature#sense` hands a brain sixteen numbers and only **nine** are about the world outside its own body — a direction and a distance to the nearest speck of food, to the nearest animal it could eat, and to the nearest one that could eat it — so the disc is a picture of the whole of an animal's knowledge, drawn in its own frame with **its nose pointing up**, because every bearing in that vector is relative to its heading and a picture with north at the top would draw a frame of reference the animal does not have. The number that made it worth building: over twelve seeds and 264,780 pond-instants, **28.3% of the animals that can see food at all have it behind them**, which is the visitor's *why is that one being so stupid* answered with arithmetic. Every mark is borrowed — `foodMote()`, `predatorOutline()`, `visionReach()`, `key.js`'s stand-in hues — so the placard two panels down is already this figure's legend. The rule it settled is v1.157's, corrected by a screenshot that caught a held caption saying *behind it on its left* over a dot that was up and to the left: **the words say what, the picture says where.** A held description of a quantity that is *also drawn live* will disagree with its own figure whatever the hold, so the visible line names only what is in sight (0.36 changes a second at 1×) and every bearing lives in the disc; the `aria-label` keeps bearings and distances, because a listener has no disc and nothing to catch the label out against. Prey and threat are gated on `config.predation` even though the senses are not — `World#step` fills those slots from `canEat`, which never asks whether the mechanic is on. Pure observer: reads the buffer the brain was handed, writes nothing, draws no random number. | `test/eyeview.test.js` |
| `decide.js` | **The step between seeing and doing.** `eyeview.js` shows what an animal is told and `doing.js` shows what it ends up doing; the only part that is actually the *animal* — the tiny brain the front door has promised since day one — had been drawn exactly once in a hundred and sixty-two releases, as a bar chart of 243 weights behind v1.149's switch. This is the picture: one horizontal line read left to right as **which way**, a green speck on the top row for where the nearest food lies with a line dropped from it, and a white bar on the bottom row out from the middle to the turn the brain is asking for. Line the two up and the animal is going for its lunch; do not, and a visitor is looking at the honest answer to *why is that one being so stupid*. The number behind it, over twelve seeds and 233,123 animal-instants with food in sight: the animals a pond is **handed** turn towards food they can see **49.7%** of the time — the arithmetic null, a coin toss — and after five thousand steps the pond manages **58.6%**. Nine points, taken once a step by every animal alive for as long as it lives, and what it compounds into is `aim.js`'s 75.2% *pointed at* their food: **a position is hundreds of decisions already added up, and this is one decision.** The rule it settled is v1.161's sibling — that panel split its words from its picture by **quantity**, and this one splits them by **time**: the line carries a running share of what this animal has done *since you picked it* — *since the pond began*, for the animal `onstage.js` seated — and the figure carries *now*, so no hold of any length can put them out of step. The share is sampled on the **step** from `witnessStep`, never on the frame, so two people who pick the same animal in the same pond read the same number; `MIN_SAMPLES = 120` is measured (the median gap to an animal's finishing share is 5.0 points at 60 samples, 3.8 at 120, 1.9 at 300). It stores nothing on a creature: rather than put two motor fields on the pond for an observer's benefit, it re-runs the brain pass with `learning: false`, which is bit-for-bit what steered the body on every fixed brain. Pure observer. | `test/decide.test.js` |
| `onstage.js` | **The animal the page sits down for you.** Three of the sentences in `hand.js`'s two-register table — `#doing`, `#eyeview`, `#decide` — existed only to say that the visitor had not picked anybody yet, and at 390 × 844 those three panels run from 978 px to 1,528 px: **65% of the second screen of this page and the whole of what a stranger meets after the pond**. Three good panels, all switched off until somebody guesses that the dots are clickable. So `openingPick` seats the living animal **nearest the middle of the water** on the frame each pond is adopted, and the three panels are alive on the first frame. Not `pickStar`, for a reason that module states about itself: at tick zero its whole ladder is empty and it falls through to *the best-fed animal right now*, a claim that is false a tick later — and the only thing that is true of a founder is where it is. No camera rides along, which is the whole difference between this and `👋 Meet somebody`: a page that zoomed a stranger into one animal before they had looked at the pond would be answering a question nobody asked. The seat is borrowed, not taken — *the seat is the page's* is spelled `renderer.selected === view.pagePick`, so a click, a tap, an arrow key, `M` or a cast row releases it with no flag to keep in step — and when the page's own pick dies it does **not** pick again, because that instant belongs to `obituary.js`. Ties to the lowest id, `cast.js#best`'s rule. Pure observer: reads positions, returns one of the animals it was handed, draws no random number. | `test/onstage.test.js` |
...
| `pondclock.js` | How this page says **when**. Three surfaces dated an event and each did it in its own dialect: the ladder `1,724 steps in`, the record board `back in year 1`, the Chronicle `t244 · yr1` — the engine's own variable with a letter in front of it, on the panel a visitor is most likely to read. A year here is `seasonLength`, 2,600 steps, about forty-five seconds: over twelve seeds run six thousand steps, **91.8% of adjacent Chronicle lines carry the same year stamp as the line above** against 7.4% for the step, 56.3% of all lines ever written say `yr1`, and a pond's whole feed sits inside one year until step 2,601 (median). So all three date in steps now, in v1.131's phrasing — *an elapsed time, not a coordinate*. `yearOf` is the one copy of arithmetic that had been spelled out by hand in `chronicle.js`, `describe.js` and `records.js`, the second of them under a comment worrying about exactly that; the year survives only where it is the right unit, on the season badge, which is about *now* rather than about when something happened. v1.138 added the clock's second question — `stepsOver`, how *long* something took — for the same reason the first one moved here: the difference between the two is exactly the word *in*, and a narrator spelling a duration where it needed one would be the third dialect starting over. Imports nothing and draws nothing. | — |
| `feed.js` | The Chronicle, as something you can **press**. Every other board here learned to point at the water — the cast list in v1.119, the record book in v1.124, the plates over the animals in v1.127, the ladder in v1.133 — and the one panel a visitor actually sits and reads stayed a wall of text; v1.125 even put a name in it, in bold, in its own element, which is what a link looks like, and eleven releases went by with nothing happening when you pressed it. Now a line about somebody still in the water goes and finds them, a line about a family that still has members lights that family up in the pond, and every other line stays a statement (v1.51's rule: a control that does nothing is worse than no control). The finding is that **the Chronicle has two kinds of subject and they decay at completely different rates**: over twelve seeds run six thousand steps and sampled every fifty, 53.6% of the lines on screen have a subject at all, and of those, 36.6% of the lines about an *animal* name one still alive against **94.3%** of the lines about a *family* — an animal is one body with a death in its future, a lineage is a population that has to lose every member at once. Together that is 51.0% of the lines with a subject, a mean of 3.63 live controls on screen, and at least one on **79.2%** of instants. And it reverses the default pond: with animals alone, seed 314 was the *worst* of twelve at 20.0% of instants with anything to press, and with families it is the **best at 93.3%** — the world every screenshot uses is a sample of one and would have talked me out of the feature. Pressability also decays down the column (97.9% under 200 steps, 93.4% at 200–600, 71.6% at 600–1,500, 32.1% beyond), and the feed is newest-first, so the live end is the top where a reader starts. The two presses that put something in the *water* wear the ladder's `👀 Show me`: one promise, two mechanisms. **v1.137 added the third.** A buried name used to be the end of the road; `memorial.js` keeps the life `obituary.js` writes at the instant of death, so the line opens their story instead — `📖 Their story`, a different verb because it is a different promise. That takes the animal lines from 36.6% pressable to **100.0%** (8,402 of 8,402 over the same twelve seeds) and the panel as a whole from 26.2% of its lines to **52.9%**, a mean of **8.06** controls on screen against 3.63. A row therefore carries its *kind* rather than a live/dead boolean: a death now changes which offer a row makes, and a panel that patches itself has to be told the difference. **v1.138 stopped it stuttering**: a run of the same sentence about the same animal is folded by `streak.js` into one row that says the newest of them and how long the run has been going, which takes the share of adjacent lines repeating their predecessor from 13.3% to 1.6%. A row is therefore no longer an event, and it compares against its predecessor on its whole painted form rather than on any one field of it — that comparison had been widened twice by a release that gave a row a new way to change. **v1.139 made it answer back**: a row about whatever the page is currently showing says `📍 You are here` where its offer was, and `aria-current` says the same thing to a listener. The mark is a comparison against the page's state rather than a memory of the press (`here.js` has the argument), which is why pressing a line about somebody lights a mean of 2.39 lines rather than the one pressed — it hands you the rest of their story in the panel you were already reading. Pure observer — it reads events and returns strings. | — |
| `memorial.js` | The book of the dead, so every name in the story is a door. Four releases running, this project's own notes ended with the same unbuilt thing: 63.4% of the Chronicle's animal lines name somebody buried, and `obituary.js` wrote a life at the instant of death and threw it away, because the only reader it had was the panel the body was already in. This keeps them. Every animal the Chronicle names is watched from the line that names them, and the life is written the step they die — per *step* rather than per frame, `trail.js`'s reason: at 20× a frame is twenty ticks and an animal named and then eaten inside one of them would be a life nobody wrote. **The measurement is the cleanest one these sweeps have produced: 100.0% of the feed's animal lines become pressable, on every seed** — and the reason it is whole rather than nearly is that the Chronicle only ever names an animal who is alive as it writes (29 named subjects over twelve seeds, none already buried), so a watcher that picks a name up on sight catches every one of those deaths. The panel's dead ends were never a gap in what could be known, only in what anybody kept. **The book has no size of its own**: a card is worth keeping exactly while a line could ask about it, so it is pruned against the Chronicle's subjects and bounded by that panel's buffer — the observed maximum is 4 cards. Bodies are lent for one call and never held (`obituary.js`'s rule). Pure observer: it reads a flag the world has already set and writes nothing back. | — |
| `streak.js` | When the same thing keeps happening, **say it once and say how often**. A champion beats their own record seven times for every once they are dethroned, so the panel a visitor actually sits and reads delivered its best story as eight copies of one sentence with a different ordinal in it — over twelve seeds run six thousand steps and sampled every fifty, **13.3% of the lines on screen were the line above them restated**. A run of adjacent lines that are the same sentence about the same animal is now one line: the newest of them, and then how long the run has been going (*Onyx raises their 12th — 6 times in a row, over 847 steps*). That takes the repeat figure to **1.6%**, folds **11.1%** of all lines, and moves the panel from a mean of 14.50 lines to 12.88; 58.3% of sampled instants have a streak on screen and the longest observed run is six. Two rules carry the design. **Only somebody can be on a streak**: grouping by sentence alone quietly collapsed the pond's own milestones — *swells past 100 creatures* and *…past 200* are one shape and 170 adjacent pairs over the sweep, two different facts wearing one sentence, where a champion's tally is one fact restated — so a run needs a `who`. And **a shape, not a prefix**: sentences are compared with their numbers blanked, so *raises their 11th* and *their 12th* match while *is the first animal here to raise 5 young* stays its own line, which is the difference between folding 17.3% of lines and folding the 11.1% that were worth folding. A folded row keeps the newest line's subject, so the press that was on the run is on the summary, and it takes its identity from the run's **first** line, so a streak growing patches one row instead of rebuilding a panel that is full of buttons. Pure function of an array of events. | — |
| `here.js` | **Which of this page's offers you have already taken.** Every board here points at something — the cast list at an animal, the ladder at a family, the plates over the water at whoever is under them, the Chronicle at all three — and not one of them has ever said which of those you *took*: press `👀 Show me` and the camera goes, the badge says `🎯 Cove`, and the line that sent you there looks exactly as it did before, still offering. Four cycles of this project's own notes ended with that sentence. The fix is one word on the control, and the design decision is what puts it there: **not a memory of the press.** There are five doors into watching an animal here (a name plate, `👋 Meet somebody`, an arrow key on the pond, a cast row, a Chronicle line), so a panel that lit up only for its own presses would sit dark in four of them while the page around it was plainly showing that animal. So the question is *is this row about what the page is showing right now?* — a comparison of two integers, true however the visitor got there, and it cannot go stale because it remembers nothing. The reframe pays: over twelve seeds sampled every fifty steps, **a press about an animal lights a mean of 2.39 lines and more than one of them 80.7% of the time** (2,328 presses, up to five at once), because whoever does something once tends to do it again — while **a press about a family lights exactly one, 2,130 times out of 2,130.** That gap is `feed.js`'s v1.136 asymmetry from the other side: a family is a durable subject the story names once, an animal a fragile one it names again and again. A module of its own rather than a private function in the panel that needed it first, because the cast board and the record book point at animals too and will want the same word — v1.131's lesson, that a rule written where it was discovered is a rule nobody else can find. Two integer comparisons and a constant string. | — |
| `postcard.js` | **The pond as something you can paste.** `🔗 Share` copied a bare URL from v1.44 to v1.139: a request for forty seconds of a stranger's attention on trust, while every fact that would have earned those seconds — a pond with a name, an age, a champion, a record crowd and a plain sentence about what is happening in it right now — sat on the page and stayed there. The press now writes the pond's story onto the clipboard with the link at the bottom of it, and puts the same words on screen, because this is the only control here whose effect lands somewhere the visitor cannot see. Four rules, three of them restraint. **A hook, not a report**: two records at most out of the board's three, and the first is the one about a *person*, because a stranger has no reason to care that a lineage peaked at 88 and every reason to wonder who Tamsin is. **A sentence written for the person at the controls is not a sentence for the person you send it to**: the headline over the water closes the card, except in the one state where it stops observing and starts instructing (*press ↻ Reset to start the pond over*), which is advice for somebody holding the keyboard. **A line with nothing in it does not appear**, so a pond thirty steps old gets two lines rather than a column of zeroes. And **nothing on it is a control** — every other narrator here composes something you can press, this one composes something you can paste, so the names are baked into the sentences rather than resolved at a press. Pure function of a world and a config. | — |
| `permalink.js` | The link, with everything nobody changed left out of it. The hash carried all of its fields whether or not anybody had touched them, so the link to the default pond was three hundred characters of which all but one said *the default* — invisible for ninety-six releases, because a URL is a thing you paste rather than a thing you read and its length is a property of somebody else's address bar. v1.140 put it at the bottom of something a person reads, and the general note is worth more than the fix: **a value nobody looks at has no quality, and the day something starts looking at it is the day its quality becomes a fact.** The shortening is exact rather than lossy — `parseHash` applies a field only when the hash carries it, so an omitted default and a written default build the identical config and therefore, by directive 2, the identical world, which `test/permalink.test.js` asserts field by field and then again through a world's state hash. Comparison is on the **written** form and not the value: `food` is serialised with `toFixed(2)`, so the question a field answers is not *is this different?* but *would writing it down change the pond the link opens?* Pure function of a config. | — |
| `picture.js` | **The pond as something you can post.** The postcard is text, and text is what a chat window renders — but the thing people actually share is a picture of the pond, and until v1.141 the only file this page could hand anybody was a **CSV**, which is a file for a person who already cares. One press now saves a PNG: the live frame at whatever camera and zoom the visitor has it at, the pond's name over it, and its own sentence and address under it. Four rules, and each is the postcard's rule on a different medium. **The picture is the pond, not a re-drawing of it** — a share that silently reframed to a stock wide shot would be handing somebody a photograph of a place they have not been. **The name plates come too**, because the pond is two canvases and compositing only the water would drop the one mark that makes a dot into somebody. **Three lines of text and one of them is the address**, because a picture is looked at rather than read. And **nothing on it is a control**, more so than the postcard: a PNG in a camera roll has no JavaScript, no hover, and no way to ask a question. Composes and paints; it is handed a context and creates no canvas of its own. | — |
| `lineage.js` | **Who this animal's parents were, by name, back to a founder.** This page had said "family" three ways for a hundred releases and meant a *population* every time — the species tree, the ancestry pips, the two-word lineage names. The one link between two individuals, `Creature.parentId`, did not exist, so `Generation 12` sat in the panel with nothing attached to it: twelve animals that lived and had young and died, none of them nameable. The store is one node per *living* creature, pruned to the living every step, and what holds a dead ancestor up is the reference its descendant's node carries — so a line is exactly as long as somebody alive needs it to be and the last of a family takes its whole history with it when it goes. That makes it the first store here bounded by the language instead of by a rule I wrote. Measured over twelve seeds and three thousand steps: the store's size equals the population on every step of every seed, families run to twelve generations, **22.8% of links are animals no longer in the water and 63.0% of families contain at least one** — an ancestor here does not have to die for its descendants to breed, which is the opposite of what I assumed and the reason the reference matters for two families in three rather than for nearly all of them. Also the words: a row of names, a sentence placing the animal in its family, and one sentence about what changed between the founder and the animal on screen — held to `creatureIntro`'s bar of no units and no jargon, because it is the next paragraph in the same panel. Pure observer; `parentId` costs no random draw and no rule reads it. | — |
| `handfeed.js` | **The water, as something you can put food into with your finger.** Twenty-three cycles went into explaining this pond to the person in front of it and by v1.146 there were more surfaces here that talk about the animals than there are ways of touching them: a visitor could read eleven panels and still only ever have paused the pond, scattered food over all of it, and pressed reset. The project's one big claim — *nobody taught them to find food* — was the one claim a stranger could not check, because food everywhere is a test of nothing. Ten pellets on a spot you chose is an experiment: **the first is taken after a median of 47 steps and the handful is gone in 198, against 589 for the same ten pellets scattered over the whole pond** (12 seeds × 4 launch points). Concentration is worth three times the clock, from the same animals with the same food, and what it demonstrates — that they *converge* — is a thing no panel here can show. A golden-angle spiral, so **not one random number is drawn while the mode is on**, which is a stronger claim than the one the opt-in features make. The measurement that rewrote it: the standing crop is at `foodMax` from tick 200 to past 1,500 and on 27.0% of all instants, so a lever that honours that ceiling does nothing during a visitor's first minute — which `✦ Feed` had been doing since v1.0. | — |
| `scenarios.js` | Curated one-click world presets (data only). Each carries a long `blurb` for the launch banner and, since v1.154, a short `hook` for the strip's caption. | `test/scenarios.test.js` |
| `worlds.js` | **The words that make thirteen worlds legible.** The strip has offered curated worlds since v1.20 and every sentence explaining them lived in a `title` attribute — a tooltip, which does not exist on a phone and costs a one-second hover on a desktop. A browser walk found **no blurb anywhere in the rendered text of the page**, and 2 of 13 chips on screen at 390 px (267 px of a 1,856 px row). The general finding: **a string in an attribute is not on the page**, and the only proof that words are on one is reading them off the rendered text. Holds `HOOK_MAX` (42 characters — the measured wrap point at 390 px is about 61, so the budget is deliberate headroom and a hook is a promise that gets worse as it lengthens), `worldsLabel` (the count, read off the array for v1.37's reason), `matchScenario`/`sameConfig` (which world a config *is* — the lamp follows the config, not the last press, because nothing had ever put it out), and `worldCaption`, which has a third answer nothing here had ever noticed: a config that is neither a scenario nor the default is a world the visitor **built**. Pure observer: no DOM, no world, no RNG. | `test/worlds.test.js` |
