...
test("every field the books carry is either hashed or excluded with a reason", () => {
  ...
  // that the world is *stepped*: six of `Stats`'s forty-three own properties are
...
test("nothing in the simulation reads the books", () => {
  ...
  // of the fifty-one fields wrong for sixty consecutive ticks and check the pond
  ...
  assert.equal(swept, 51, "the books changed size; the claim above needs re-measuring");
});
