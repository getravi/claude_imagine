...
- **And one button hands you the thing that takes time.** Evolution here moves
  at about a generation every four hundred steps, which is the one ingredient
  nobody browsing at eleven at night has any of. Press **⏩ Skip ahead** (or
  <kbd>S</kbd>): the pond runs 2,600 steps in about three seconds — the water
  visibly racing, not frozen — and then a card opens on **what shape that
  stretch was**, in four words and a picture. *A boom, then a crash. The crowd
  swelled from 41 to 310 about halfway through, then fell back to 168* — over a
  small line drawing of the whole year with the peak pinned on it. Under that,
  what it cost: *434 were born and 307 died while you were away, 27 of them
  eaten. 9 more generations have been born. The animals here are 23% bigger
  than the ones you left behind. Meat has fallen from 54% of what they eat to
  18%.* Then the Chronicle's own lines from the stretch you skipped.
  **⏩ Skip again** goes round for another.
...
| **The pond's name** (above the water) | Every world here had an identity and it was an integer. The field says `Seed`, the permalink says `#seed=42`, and nobody has ever told a friend about seed 1837465, or come back to one of three browser tabs all reading "Vivarium" and known which pond was which. So a seed is now a **place**: seed 314 is *Western Mere*, seed 42 is *Sleeping Millpond*, and the tab says so too. It is a pure function of the seed and of nothing else — not of the sliders, which move continuously and would rename the pond under a dragging finger — so the same link always arrives somewhere with the same name. The vocabulary is adjective-plus-landform, deliberately disjoint from the noun-plus-noun the lineages use, so no pond can read as kin to the *Slate Darts* swimming in it. And it is a handle rather than an identifier: 48 × 32 = 1,536 names, the first repeat at **seed 62**, which is why the seed stays printed beside it. |
| **🎲 Another pond** (beside the name) | The nameplate has said *the same seed always grows the same pond* since v1.134, and for forty-one releases the obvious next thought had nowhere to go: there are 1,536 named ponds behind that sentence and the only door to them was a 36 × 33 die beside a number field, **5,050 px down a 5,709 px page** on a phone — 88.5% of the way, the 34th of 45 targets a thumb passes. One press beside the sentence takes you somewhere you have not been, the water says *🪷 Welcome to Whispering Vale*, and **↩ Western Mere** appears beside it so the place you just left is always one press away. The rule underneath is the one nobody had costed: two seeds can share a name, so over two million draws **20 presses of the old die land somewhere twice in 12.8% of sessions and 40 in 41.7%**, while 1 press in 1,536 lands on the name already on the plate and says nothing at all — the page greets you only when the *name* moves, so that press looked exactly like a broken button. The door keeps a log of where you have been and will not offer it twice, at a measured 1.0137 draws a press. |
| **Share 🔗** | **The postcard.** For ninety-six releases this copied a bare URL, which is a request for forty seconds of a stranger's attention on trust — while every fact that would have earned those seconds stayed on the page. Now it copies the pond's *story*, with the link at the bottom of it, and shows you the same words before you send them: the pond by name, how old it is and who is in it, what it has been through, its two best records, and one plain sentence about what is happening right now. Over twelve seeds sampled through six thousand steps it is a mean of **4.91 lines and 469 characters**, and **86.5%** of them name a person — *Cove raised 12 young — gone now, and unbeaten since.* The link inside it got shorter too: the hash used to write all twenty-nine permalink fields whether or not you had touched any of them, so the default pond's URL was **252 characters of which all but one said *the default***. It is **54** now, and a scenario's is 64. Exactly the same pond opens either way. |
| **📸 Take a picture** | The only file this page could hand anybody used to be a **CSV**, which is a file for a person who already cares. This one is for the person who does not yet: one press saves a PNG of the pond — the frame on screen, at the camera and zoom you have it at, with the name plates over the animals still on it — under the pond's name and its age, and over the sentence the page is telling and the address to go and see it. It is the *live* frame, so pause first if you want a particular moment, and it saves at the water's own resolution rather than at the size your window happens to be showing it, so a picture taken on a phone is the same picture as one taken on a desktop. |
| **🎞 Make a GIF** (<kbd>G</kbd>) | The same picture, moving — which is the version of it anybody actually wants. A still of this page is a still of some dots; everything Vivarium is *about* happens over time, and no photograph and no paragraph carries it. One press records **48 frames, two steps of pond apart**, and hands back a looping animated GIF of just under two seconds: the water as you have it framed, name plates and all, under the pond's name and its live numbers — the population and the step count count up inside the file — and over the address. It plays by itself in every chat window, feed and phone gallery on earth, which is the entire reason it is a GIF and not a video. The file is about **2 MB**, and the press takes about four seconds: a second and a half recording, then the rest compressing, with the button counting through both. The encoder is written from nothing in [`src/gif.js`](src/gif.js) — colour table, LZW and all — because this project has never had a dependency and was not about to start. Like a skip, a recording drives its own stepping, so a GIF made on a slow laptop and a fast desktop is the same two seconds of pond. |
| **🧭 Show me around** (<kbd>?</kbd>) | The page, introduced in six stops. A ring lands on the pond, then on the line above it, then on the placard that names the marks in the water, then on **👋 Meet somebody**, then on **👁 What it can see** — the disc holding everything one animal knows, with **🧠 What it decides** under it — and finally on **⏩ Skip ahead**, each with two sentences saying what it is and why it is worth a look. That fifth stop was *the row of other worlds to try* until v1.166, and the swap is the rule the guide is now held to: **a stop is spent on what a visitor would not find alone.** Measured on a 390 px phone, the row of worlds sits at 176 px and is on screen before anybody touches anything, while the two panels that say what an arrowhead actually *is* begin at 1,097 px — 313 px past the bottom of the only screen most visitors ever see. The row also stopped needing a stop two releases before it lost one: it earned its place in v1.129 when a chip was a bare noun, and v1.154 gave every chip a sentence of its own. When a panel learns to explain itself, the guide's stop on it is the thing to re-cost. The last card is the only one that *does* something: it carries a **⏩ Try it** button that closes the guide and runs the year, because a final card reading *now go and press this* is asking a stranger to find a control the card is sitting on top of. It opens itself once, on a first visit, and every way out of it (Skip, Done, <kbd>Esc</kbd>, a press outside the ring) remembers that it ran. The scrim is the interesting part: dimming everything outside the ring is what every guide of this shape does, and measured against this page's own grounds it moves them by a contrast ratio of **1.012 to 1.121** — nothing at all, where the same veil over a white page moves it by 9.32. What it does dim is the one bright thing here, the paint inside the canvas; on every other stop the pointing is done by the ring alone. |
| **🙂 Simple view / 🔬 Everything** (the switch in the top bar) | The page starts with the **instruments put away**. A first-time visitor meets the pond, the sentence over it, the verb under it, the key to the water, the ladder, the stand-outs, how they have changed, the records, the Chronicle, what they die of, and eleven buttons — and none of the apparatus. One press brings back the other 34 dials, the 5 figures, the 36-field fact grid and the Tree of Life, and the browser remembers which side you chose. Measured: the front of this page is **4,075 px tall at 1,280 px wide and 2,569 px in Simple — 37% of it gone** (38.8% on a 390 px phone, 7,350 px down to 4,498). `🧭 Show me around` was v1.129's answer to the same sentence and it is a different one: a guide **ranks** a crowded page, it does not uncrowd it, and the visitor who never presses it — which is most of them, because nobody knows they need a guide until they have already decided to leave — still meets all of it at once. The switch says what is behind it, and the counts on it are read off the page at runtime rather than typed, so a rule added next release cannot make the label a lie. Nothing is removed, only hidden: the chart you ask for after four minutes already holds four minutes. And the preference is deliberately **not** in the permalink — a link carries a world, so a pond you share opens the way *you* prefer, not the way the sender does. |
| **👋 Meet somebody** (<kbd>M</kbd>) | Hands you an animal instead of asking you to find one. The pond is ranked by how much of a story each creature has — the last surviving member of a family that was once several, the parent of more of the pond than anyone else, the biggest hunter, a giant, the oldest, and, when nobody has a story, whoever is best fed — and the winner is selected, followed and introduced by name in one plain sentence. The choice is arithmetic, not a coin: the same pond gives back the same animal. Since v1.153 it sits in a row directly **under the water**, beside **⏩ Skip ahead** and **🧭 Show me around**, rather than in the panel on the right: on a phone that panel is below the whole page, and this button — the one this page recommends to a stranger — was measured at **3,692 px down a 4,815 px document**, past eleven panels. It is at 685 px now, and the three of them are the first controls here to clear WCAG 2.2's *enhanced* 44 px target bar. |
| **⏩ Skip ahead** (<kbd>S</kbd>) | The one control here that answers *what did I miss?* rather than *what is happening now?*. Evolution in this pond moves at about a generation every four hundred steps, and every surface built to narrate it — the headline, the Chronicle, the record board, the board that says how far these animals have drifted from their founders — still needs the visitor to supply the waiting. One press supplies it: the pond runs **2,600 steps** (its own year, `config.seasonLength`) in about three and a quarter seconds, drawn all the way so the water is visibly racing rather than frozen, and then a card says what changed. It opens on **the shape of the stretch** (v1.145) — a verdict in four words, the crowd drawn as a line with its turning point pinned, and the same verdict with the numbers in it — because the sentences under it compare two instants and two instants cannot see a middle: a pond that tripled by halfway and gave a third of it back reads, to a difference, as *84 were alive when you pressed it, and 154 are now*. Under the shape: who was born and who died, how many generations went by, and how far the bodies and the diet moved, at the same thresholds the **🧬 How they have changed** board uses. Then up to three of the Chronicle's own lines from the stretch you skipped, and a count of the ones that did not fit. The threshold a shape has to clear is a fifth of the crowd, which is the middle of the widest band the same 60-stretch sweep leaves standing: at 0.15 the *quiet* verdict never once fires in sixty stretches, and at 0.40 the *boom* verdict never fires either — a pond that went 40 → 231 → 154 is announced as *it grew*, directly above a drawing with a hill in it. The length is not a number this page picked: over 12 seeds × 5 launch points, every one of 60 skips of 2,600 steps had at least one thing to report, where **4 of 60** skips of half that came back with nothing at all. A skip is waiting done faster and nothing else — the same `World.step` the running pond makes, the same number of times whatever machine you are on, so a skipped pond is bit-for-bit a pond that was left running. |
...
| **🏁 Day one vs today** (under the pond) | The one claim on this page you can check without taking my word for it. Every other answer to *have these things actually evolved?* is a reading — a mean, a share against a coin toss, a plot for somebody who already knows what a plot is. This is the same question with a finish line under it: ten animals from this pond's **first moments** in one small pond, ten from **right now** in an identical one, eighty specks of food in each and nothing more ever growing. Same seed, so the food falls in the same places and every animal starts on the same spot facing the same way; nothing hunts in the lanes, nothing is topped up when a team dies. The only difference between the two worlds is what is inside the heads of the ten animals in them — and then you watch. Measured over 12 seeds × 3 ages, **today's animals won 34 of 36**: they clear the eighty in a typical two to four seconds, while day one's animals cleared theirs at all in only 9 of the 36 and usually ran out of time with **half the food still floating**. Press it on a pond nobody has run yet and it is a dead heat, which is the fairness of the lanes proving itself. The two lanes are separate worlds with their own random numbers, so a visitor who races is watching, bit for bit, the same pond as a visitor who never presses the button. |
| **Names on the water 🏷️** | The pond's names, on the pond. Six releases went into teaching this page to call things by name — the lineages, the star, the cast board, the record book, a Chronicle that says *"Marlow raises their 6th"* — and every one of those names lived in a panel, so a visitor who read one looked up at three hundred identical darts with no way of finding Marlow. Now the handful of animals this page has a reason to name wear a small plate with their given name and the mark of what makes them worth watching, and the **🏅 Worth watching** board under the water is the key to it: same animals, same marks, same order, one list. The plate is drawn after the lens comes off, so a name is the same size at 8× as at 1×, and it is opaque, which is what lets its ink be measured — 16.6:1, against a bar of 4.5. No hold and no fade, because the churn everybody assumes is there is not: over six seeds and 6,000 ticks the cast changes a mean of **41 times, one change every 146 ticks**, since every role here is an extremum over a slow quantity rather than a threshold on a live share. **And since v1.150 a plate says what its animal is doing** — *🏆 Marlow · fleeing*, *Nim · after food* — in a quieter ink beside the name, out of the same list of nine states the **What they are doing** line reads, held by the same clock and looked up once, so the plate over a dart and the strip under the pond can never say two different things about one animal in one frame. That is the difference between a pond that tells you a story and a pond that waits to be asked: the plates hang over three or four animals **nobody has to pick**, so a visitor who has not yet worked out that the darts are clickable still watches somebody flee, hunt and eat by name. **Press a name and you go and watch that animal** — the same thing pressing its row on the board does, because it is the same list. A plate is 62 × 24 page pixels wherever it is drawn, against the 11-pixel circle a creature is caught by on a phone, which makes it the one thing in this water a thumb can reliably hit; and a press on it would have caught **nobody at all 75.7% of the time** and *somebody else* 20.2%, because a name floats above its animal in open water rather than on top of it. |
| **🌱 How far this pond has got** (under the pond) | The only surface here that points *forwards*. Ten things a pond does as it grows up — the first young, one animal eating another, a family taking hold, a dynasty, twice as full as it started, ten generations deep, a new kind of animal, five hundred lives, fifteen generations, a thousand lives — ticked with how far in each one happened, or still ahead carrying the live number it is standing at: *the busiest parent so far has raised 3 of the 5 it takes*. Everything else on this page is a reading of now or a record of then, so a visitor who arrives at a two-minute-old pond is shown an empty record book, a cast of animals that all look alike and a plot with one band in it, and reads the honest sum of that as *nothing happens here* — when the pond is nine steps from its first birth. Two rows you would expect are missing and that is the finding: *the founders are all gone* lands on step **4,200 on eleven ponds of twelve**, and so does *somebody dies of old age*, because both of them are the maximum lifespan wearing a rosette. What survived varies from pond to pond, which is the only way a milestone is about the water rather than about the rules. And the ladder found one thing nothing else here had said: **on 11 of 12 ponds the first animal to die and the first animal to be eaten are the same animal on the same step** — the opening event of a pond is a killing. Five of the ten rungs are about an *animal* rather than about a pond, and those five are pressable: press **👀 Show me** and the camera goes and finds them — the family's newest member, the animal that has raised the most young, the one furthest from the founders, the newest animal of the new kind. The others stay text, because a control that does nothing is worse than no control. **The last four rungs are v1.180**, and they exist because of one stopwatch: left alone on the default pond this panel said `6 of 6 so far` after sixty seconds and for the whole of the four minutes after it, so the page's one forward-pointing surface spent 80% of a visit as a finished list. The second half was swept over 20,000–24,000 steps instead of 6,000 — and the horizon is the whole finding, because *twenty generations*, rejected by v1.131 at 0 of 12, turns out to fire on **21 of 22** ponds over the longer run: a rung that fails on one pond in twenty-two fails invisibly, which is worse than one that fails on all of them. Fifteen is 22 of 22. |
| **The pond cheers 🎉** | The other half of that ladder. When a rung is actually climbed, the pond says so — a banner over the water naming what just happened in one sentence and what to wait for next (*🌿 A family takes hold — one bloodline is now big enough to have a name of its own. Next: a dynasty.*), and the ladder below the pond lights up so you learn where this page keeps its progress. A checklist nobody congratulates you for finishing is a tax form. It cannot be noisy: twelve ponds over six thousand steps climb the ladder in 69 separate moments — about six banners a run, one every five hundred steps — and a pond restored from **Load** stays quiet about the life it was saved from. |
| **The pond's news 📜** | The Chronicle, on the water. The best writing on this page has always lived in the sixteenth panel: a family takes the whole pond, the hunters die out, a sickness burns itself out, a family splits off that this pond grew itself — all of it four thousand pixels below the animal it happened to. The biggest of those moments now come up over the water in a sentence written for somebody who has scrolled nowhere (*🌿 A new family has appeared — the Shale Fins, and this pond grew them itself.*), the panel they came from lights up behind them, and the ones about somebody carry **👀 Show me** — press it and the page goes and finds them, or lights their family up in the water. **It is an editor rather than a pipe**, and a measurement is why: the feed writes 23.6 lines per six thousand steps, which at the speed the page opens is a line every 4.2 seconds into a banner that stays up for 5.2 — piped straight through, the story would cover the pond it is about and never come off. So the water takes the best moment of each quiet stretch, ranked, and forgets the rest; twenty-two kinds of moment are worth it and eleven stay in the panel with the reason written beside each — the first kill and the tenth generation because the ladder already says them, the leading cause of death because a change in a running average is not a moment, dawn because *a pond that has gone dim looks broken and the light coming back explains itself*. The sentences are rewritten rather than quoted, because the feed's reader is reading a history and the water's is watching animals, and a line may only be said twice when it is about a **role** that changes hands — the pond's best parent, the family that holds the water. |
...
| **One clock** (everywhere a thing is dated) | Three panels here say *when* something happened, and until v1.135 each said it differently: the ladder *1,724 steps in*, the record book *back in year 1*, the Chronicle `t244 · yr1` — the engine's own variable with a letter in front of it, on the panel a visitor is most likely to read. A year here is 2,600 steps, about forty-five seconds, so it was barely a date: over twelve seeds run six thousand steps, **91.8% of Chronicle lines carry the same year stamp as the line above them** (7.4% for the step), and a pond's whole feed sits inside one year until step 2,601. All three now date in **steps in** — an elapsed time rather than a coordinate — so a rung, a record and a line in the story can be read against each other. The year is still on the season badge, where it is a statement about *now* and the right unit for one. |
| **What they are doing 🌿** (under the pond) | The one line on this page with a **verb** in it. Everything else here says what a creature *is* — generation 14, 61% fed, size 4.2, calling 0.31 — and a person watching an animal wants to know what it is **doing**: *Nim is heading for food*, *Marlow is running from something bigger*, *Iris has just eaten*. Nine states, read off the animal's **own senses** — the input vector its brain is run on that tick, so *heading for food* means food this creature can see rather than food a god's-eye query found — and a meal detected from outside the simulation entirely, because there are exactly three lines in the world that add to a creature's energy and all three are somebody eating. Two of the ten states drafted for it do not exist: *right on top of a pellet* and *ready to breed* each fired on **0.0%** of 52,841 sampled animal-instants, because each is defined by a threshold the world acts on in the same tick it becomes true. The line is **held for 1.5 seconds** — the raw state changes every 14.5 ticks, which at 1× is a caption rewriting itself four times a second — and the hold is the one constant here stated in *milliseconds* rather than ticks, because what it protects is a reader's eye and an eye does not speed up when the speed slider does. Nothing is allowed to jump the hold, measured: letting the chases and the meals preempt it costs 13.8 captions per 1,000 ticks instead of 9.9 **and** 53.3% staleness instead of 44.2%, since the dramatic states are the briefest. The hold amplifies them anyway — *has just eaten* is 0.6% of the truth and 5.6% of what is shown. **v1.150 put the same verbs on the water**, in a short form (*fleeing*, *after food*, *in danger*) on the name plates over the animals — because this line needs somebody to have been picked, and for the first minute of every visit nobody has picked anybody. **And v1.170 made those plates readable**: the season badge sits in the top-left corner of the water and a plate lands there whenever its animal swims up, so a third of the time on a phone the name was drawn *underneath* it and what reached you was `…m · after food`. Names now step clear of everything the page has put on the water — the badge, the ruler, the minimap, the zoom pill and the banner — read off the stage every frame rather than from a list, so a sixth mark is dodged the day it is added. **And since v1.167 the panel has a name of its own** — **🏊 What it is doing** — which it had gone nineteen releases without: it is the first of the three boxes under the water that are one thought about one animal (*doing*, then **👁 What it can see**, then **🧠 What it decides**), and it was the only one of the three with no heading, so the page's own contents could not offer it and a screen reader's heading walk could not reach it. The card that replaces it when that animal dies gained one too, and its heading is the animal's own name — the only entry in this page's contents that is a *who* rather than a *what*, appearing the moment they die and gone again when you pick somebody else. |
...
- **Start a fresh world and just wait.** For the first ~30 seconds the pond
  ...
  is the whole point.
- **Or don't wait: press ⏩ Skip ahead.** Three seconds of the water running
  fast, and then a card telling you what happened — first the *shape* of it
  (*a boom, then a crash*; *a crash, then a comeback*; *a quiet stretch*) with
  the year drawn as a line, then the births and the deaths, the generations
  that went by, how much bigger the animals got and what happened to their
  appetite for meat, with the Chronicle's own lines from the stretch
  underneath. Press **⏩ Skip again** from the card and watch the same pond,
  four presses in, become somewhere you would not recognise.
...
- **Follow the Chronicle.** Below the pond, the natural-history feed narrates the
  ...
  Every line is dated in the pond's own clock — *1,664 steps in* — which is the
  same clock the ladder and the record book use, so the three panels can be read
  against each other.
...
- **Find a great world and Share it.** You get a postcard: the pond's story in
  five lines with the link at the bottom, on your clipboard and on the screen so
  you can read it before you send it. Whoever opens the link watches the very
  same pond evolve.
- **When it looks good, press 📸 Take a picture.** A PNG lands in your downloads:
  the water exactly as you have it framed, name plates and all, with the pond's
  name over it and its own sentence and address under it. It is the frame on
  screen, so pause first if you want a particular moment.

  ![A saved picture of Western Mere: the pond's name and numbers above the water, the animals wearing name plates, and one sentence and the address below](docs/screenshots/picture.png)

- **Then press 🎞 Make a GIF and send *that*.** Four seconds later a looping
  two-second animation of your pond lands in your downloads, poster and all —
  the same frame you were watching, actually swimming. It is the one file this
  page makes that shows a stranger what the place is like rather than telling
  them.
...
src/
  ...
  nametag.js        who is wearing their name over the water, and what they are doing
